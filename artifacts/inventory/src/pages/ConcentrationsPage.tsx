...
import { Edit2, Droplets, Plus, Search, Trash2, X } from "lucide-react";
...
export default function ConcentrationsPage() {
  ...
  const { data: concentrations = [], isLoading } = useQuery<Concentration[]>({
  ...
  const createConcentration = useMutation({
    mutationFn: async (payload: { name: string }) => {
      const res = await fetch(`${BASE_URL}/api/concentrations`, {
        method: "POST",
        credentials: "include",
      ...
      if (!res.ok) {
        ...
        throw new Error((body as { error?: string }).error ?? "Failed to create concentration");
      }
      return res.json();
    },
    onSuccess: () => {
  ...
  const updateConcentration = useMutation({
    mutationFn: async ({ id, payload }: { id: number; payload: { name: string; is_active: boolean } }) => {
      const res = await fetch(`${BASE_URL}/api/concentrations/${id}`, {
        method: "PUT",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      ...
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
        throw new Error((body as { error?: string }).error ?? "Failed to update concentration");
      }
      return res.json();
    },
    onSuccess: () => {
  ...
  const deleteConcentration = useMutation({
    mutationFn: async (id: number) => {
      ...
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
        throw new Error((body as { error?: string }).error ?? "Failed to delete concentration");
      }
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["concentrations"] }),
  });

  const filtered = concentrations.filter(
  ...
  const openEdit = (concentration: Concentration) => {
    setEditingConcentration(concentration);
    setName(concentration.name);
    setIsActive(concentration.is_active);
    setError("");
    setIsModalOpen(true);
  };
  ...
  const handleSave = async () => {
    if (!name.trim()) {
      setError("Concentration name is required");
      return;
    }
    setError("");
    if (editingConcentration) {
      await updateConcentration.mutateAsync({
        id: editingConcentration.id,
        payload: { name: name.trim(), is_active: isActive },
      });
    } else {
      await createConcentration.mutateAsync({ name: name.trim() });
    }
  };
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-60 text-center">
          <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mb-4">
            <Droplets className="w-9 h-9 text-gray-300" />
          </div>
          <h3 className="text-xl font-bold text-gray-900 mb-2">
            {search ? "No concentrations found" : "No concentrations yet"}
          </h3>
          <p className="text-gray-500 text-sm max-w-xs mb-6">
            {search
              ? `Nothing matching "${search}".`
              : "Add concentrations like EDP, EDT, Parfum to use in product forms."}
          </p>
          {!search && (
            <button
              onClick={openAdd}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          {filtered.map((concentration, index) => (
            <motion.div
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-gray-900 truncate">{concentration.name}</p>
                  <p className={`text-xs mt-0.5 ${concentration.is_active ? "text-green-600" : "text-gray-400"}`}>
                    {concentration.is_active ? "Active" : "Inactive"}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                <button
                  onClick={() => openEdit(concentration)}
                ...
                <button
                  onClick={() => {
                    if (!window.confirm(`Delete concentration "${concentration.name}"?`)) return;
                    deleteConcentration.mutate(concentration.id);
                  }}
            ...
            >
              <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-100">
                <h2 className="text-xl font-semibold text-gray-900">
                  {editingConcentration ? "Edit Concentration" : "Add Concentration"}
                </h2>
              ...
              <div className="p-4 sm:p-6 space-y-5">
                <div className="space-y-2">
                  <label className="text-sm font-medium text-gray-700">Concentration Name *</label>
                  <input
                    value={name}
                    ...
                    placeholder="e.g. EDP, EDT, Parfum, EDC"
                    autoFocus
                  />
                </div>
                ...
                {editingConcentration && (
                  <div className="flex items-center justify-between rounded-xl border border-gray-200 bg-gray-50 px-4 py-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">Inactive concentrations won't appear in product forms</p>
                    </div>
              ...
              <div className="p-4 sm:p-6 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
                ...
                <button
                  onClick={handleSave}
                  disabled={createConcentration.isPending || updateConcentration.isPending}
                  className="px-6 py-2.5 rounded-xl bg-black text-white font-medium hover:bg-gray-800 transition-colors disabled:opacity-60"
                >
                  {editingConcentration ? "Save Concentration" : "Create Concentration"}
                </button>
              </div>
            </motion.div>
          </div>
