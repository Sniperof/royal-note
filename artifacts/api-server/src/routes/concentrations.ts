...
router.post("/", requireAuth, requireAdmin, async (req, res): Promise<void> => {
  ...
  if (!name?.trim()) {
    res.status(400).json({ error: "Concentration name is required" });
    return;
  }
  try {
    const normalizedName = normalize(name);
    const duplicate = await pool.query(
      `SELECT id FROM concentrations WHERE normalized_name = $1 LIMIT 1`,
      [normalizedName],
    );
    if (duplicate.rows.length > 0) {
      res.status(409).json({ error: "Concentration already exists" });
      return;
    }
    const result = await pool.query(
      `INSERT INTO concentrations (name, normalized_name, updated_at)
       VALUES ($1, $2, NOW())
...
router.put("/:id", requireAuth, requireAdmin, async (req, res): Promise<void> => {
  ...
  if (Number.isNaN(id)) {
    res.status(400).json({ error: "Invalid concentration id" });
    return;
  }
  if (!name?.trim()) {
    res.status(400).json({ error: "Concentration name is required" });
  ...
  try {
    const normalizedName = normalize(name);
    const duplicate = await pool.query(
      `SELECT id FROM concentrations WHERE normalized_name = $1 AND id <> $2 LIMIT 1`,
      [normalizedName, id],
    );
    if (duplicate.rows.length > 0) {
      res.status(409).json({ error: "Concentration already exists" });
    ...
    if (result.rows.length === 0) {
      res.status(404).json({ error: "Concentration not found" });
      return;
    }
...
router.delete("/:id", requireAuth, requireAdmin, async (req, res): Promise<void> => {
  ...
  const result = await pool.query(`DELETE FROM concentrations WHERE id = $1 RETURNING id`, [id]);
  if (result.rows.length === 0) {
    res.status(404).json({ error: "Concentration not found" });
    return;
  }
