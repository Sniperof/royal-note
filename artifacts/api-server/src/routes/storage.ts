...
// ─── Local upload root ────────────────────────────────────────────────────────
// Resolved once at module load from LOCAL_UPLOAD_DIR.
...
// so at runtime this value is always a valid absolute path.
function getLocalUploadRoot(): string {
  const dir = process.env.LOCAL_UPLOAD_DIR;
  if (!dir || dir.trim() === "") {
    // Should never reach here — validateStorageConfig() already threw at startup.
    throw new Error(
...
// ─── GCS / Replit object storage — only active when REPLIT_ENV=true ──────────
// Outside Replit the Google Cloud Storage sidecar is not available.
// All uploads go directly to the local filesystem via LOCAL_UPLOAD_DIR.
...
// Client sends JSON metadata → server returns { uploadURL, objectPath }.
// Client then PUTs the file directly to uploadURL.
// ─────────────────────────────────────────────────────────────────────────────
router.post("/storage/uploads/request-url", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (isReplit) {
      // ── Replit/GCS path ──────────────────────────────────────────────────
      // Only used when the Replit sidecar is available (REPLIT_ENV=true).
      ...
      // Uploads go to LOCAL_UPLOAD_DIR — a persistent directory outside the app.
...
// The client is directed here only when REPLIT_ENV is false (local filesystem).
// ─────────────────────────────────────────────────────────────────────────────
...
// object_path values of the form "/local-uploads/<filename>" are resolved here.
// ─────────────────────────────────────────────────────────────────────────────
...
//
// Replit/GCS only. Returns 503 outside Replit.
// ─────────────────────────────────────────────────────────────────────────────
